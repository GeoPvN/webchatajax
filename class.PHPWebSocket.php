...
class PHPWebSocket{
	...
	*/
	...
	function PHPWebSocket($WS_MAX_CLIENTS){
	   $this->WS_MAX_CLIENTS_PER_IP = $WS_MAX_CLIENTS;
	}
				...
				foreach ($changed as $clientID => $socket) {
					if ($clientID != 0) {
						...
						if ($client !== false) {
							...
							if ($result !== false && $this->wsClientCount < self::WS_MAX_CLIENTS && (!isset($this->wsClientIPCount[$clientIP]) || $this->wsClientIPCount[$clientIP] < $this->WS_MAX_CLIENTS_PER_IP)) {
